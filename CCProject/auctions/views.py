...
@login_required(login_url="login")
def bid(request, auction_id, end_date):
    # Check to handle POST method only
    if request.method == "POST":
        try:
            auction = Auction.objects.get(pk=auction_id)
        ...
        highest_bid = Bid.objects.filter(auction=auction_id).order_by("-bid_price").first()
        watchlisters = Watchlist.objects.filter(auctions__id=auction_id).values_list('id', flat=True)
        print(f"For Auction ID as {auction_id} are {watchlisters}")
                ...
                if bid_price > auction.starting_bid and bid_price > highest_bid_price:
                    ...
                    if is_highest_bidder:
                        ...
                        if highest_bid is not None:
                            ...
                            sendEmail(email,auction.title, bid_price,0)
                            for bid_id in watchlisters:
                                if bid_id != highest_bid.bider_id:
                                    ...
                                    sendEmail(email,auction.title, bid_price,3)
...
def sendEmail(email, title ,new,choice):
    ...
    if choice == 0:
        ...
        subject = "Auction Listing Closed"
        body = "Hello Auctioner, we have exciting updates about the auction! The auction for the item "+title +" has closed. The winner of the auction is you! Congratulations! You have won the item for $"+ str(new) +". Thank you for your enthusiasm! Sincerely, Auctioner Team"
    elif choice == 2:
        subject = "Auction Listing is about to close"
        body = "Hello Auctioner, we have exciting updates about the auction! The auction for the item "+title +" is about to close. The current bid price is $"+ str(new) +". Feel free to return to our application and consider placing a higher bid if you still have your eye on the item. Thank you for your enthusiasm! Sincerely, Auctioner Team"
    elif choice == 3:
        subject = "Auction Listing Updates"
        body = "Hello Auctioner, we have exciting updates about the auction! The auction for the item "+title +" has been updated. The current bid price is $"+ str(new) +". Feel free to return to our application and consider placing a higher bid if you would like to get the item. Thank you for your enthusiasm! Sincerely, Auctioner Team"
